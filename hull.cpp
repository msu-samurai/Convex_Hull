...
int main(void){
    ...
    if( degeneracy(points, n) == 1 )
    {
		...
		list < Point > L;
		L.push_front(points[i_min]);
        L.push_back(points[i_max]);
        delete [] points;
        std::cout<<"Final list:"<<endl;
	    for (auto t = L.begin(); t != L.end(); t++) (*t).print();
	    return 0;
	}
    ...
    for(int i = 2; i < n; i++)
    {
    	...
    	//std::cout<<"upper list after i:"<<endl;
 	    //for (t = L_upper.begin(); t != L_upper.end(); t++) (*t).print();
 	    //std::cout<<endl;
    	while ( L_upper.size() > 2 && riht_turn(L_upper) != 1 )
    	{
    		t = L_upper.end();
    		advance(t, -2);
    		L_upper.erase(t);
    ...
    for(int i = n-3; i >= 0; i--)
    {
    	...
    	while ( L_lower.size() > 2 && riht_turn(L_lower) != 1 )
    	{
    		t = L_lower.end();
    		advance(t, -2);
    		L_lower.erase(t);
